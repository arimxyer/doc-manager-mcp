...
This reference file helps you stay up-to-date with speckit framework improvements by checking for updates from the upstream repository and selectively applying them.
...
3. **Check mode** (when `--check` or `-c`):
   ...
   - If updates available:
     ...
     References:
       ✓ .claude/skills/speckit/references/speckit.plan.md
...
5. **Diff mode** (when `--diff <component>`):
   ...
   - Component can be: `all`, `scripts`, `templates`, `references`, or specific filename
   ```
   File Diffs
   ==========
...
6. **Pull mode** (when `--pull <component>`):
   ...
   Updating files...
     ...
     Updating assets/spec-template.md... ✓
     Updating .claude/skills/speckit/references/speckit.plan.md... ✓
