...
description: (Add'l Step) Archive older or completed specs to specs/archive/ directory
---
...
This reference file documents the archive workflow that archives completed or old spec directories to `specs/archive/` to keep the active specs directory clean and organized.
...
3. **List mode** (when `--list` or `-l` or no arguments provided):
   - Run `scripts/archive-specs.sh --json --list` from repo root
...
4. **Archive specific specs**:
   ...
   - Run `scripts/archive-specs.sh --json [spec numbers]` from repo root
